...
namespace XferPrintLib.Configuration
{
    ...
    public class XMLBaseObject
    {
        ...
        public virtual void Save(string filePath, string fileName)
        {
            try
            {
                //logger.Debug("entering {0}.{1} = {2}", MethodBase.GetCurrentMethod().DeclaringType.Name, MethodBase.GetCurrentMethod().Name,fileName);
            ...
            {
                //logger.Debug("exiting  {0}.{1} = {2}", MethodBase.GetCurrentMethod().DeclaringType.Name, MethodBase.GetCurrentMethod().Name, fileName);
            }
        }
        ...
        public StringCollection GetSerializedSchemaValidationErrors(string recipeSchema)
        {
            var collMessages = new StringCollection();
            try
            {
                var xdoc = new XmlDocument();
                xdoc.LoadXml(AsXMLString());
                var schema = XmlSchema.Read(new StringReader(recipeSchema), null);
